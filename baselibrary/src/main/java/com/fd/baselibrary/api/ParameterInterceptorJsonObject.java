 ...
 * 参数格式为Json格式的拦截器
 */
public class ParameterInterceptorJsonObject implements Interceptor {
    private static final String TAG = ParameterInterceptorJsonObject.class.getSimpleName();
...
//        Log.e("token", SPManager.getLogin_token());
        if ("GET".equals(method)) {
            ...
            request = request.newBuilder()
                    .url(newUrl)
                    .addHeader(SPManager.getUserTokenKey(), SPManager.getUserToken())//添加header
                    .build();
            ...
            if (oldRequestBody instanceof FormBody) {
                ...
                for (int i = 0; i < oldBody.size(); i++) {
                    ...
                    try {
                        if (oldBody.name(i).equals("json")){
                            String s =oldBody.value(i);
                            Log.e("111111111", s);
                            if (json==null){
                            ...
                            json.put(oldBody.name(i), oldBody.value(i));
                        }
                ...
                //构建一个新的请求
                Log.e("11111111",SPManager.getUserTokenKey()+"");
